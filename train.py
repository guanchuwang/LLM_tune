...
ROOT = '/home/gw22/python_project/Sparse_LoRA/saved_models/soft_prompt_llama'
...
parser.add_argument("--full_tune", action="store_true")
parser.add_argument("--lora_dim", type=int, default=16)
parser.add_argument("--prune_level", type=int, default=1)

def prune_model(model, func=1):

    targets = ["q_proj.weight", "k_proj.weight", "v_proj.weight"]
    ...
    if func == 0:
        ...
        for n, p in model.named_parameters():
            if any(t in n for t in targets):
                ...
                p[:] = p.data * torch.eye(p.shape[0]) # here p[:] = ... is necessary, cannot use p = ...
                print(f"{n} is sparsified...")
...
def prepare_input_and_label(model, inputs_ids):
    ...
    #     model = model.module
    padded_input_tokens = inputs_ids # model._extend_labels(inputs_ids)
...
# @torch.no_grad()
# def evaluate(model, valenc, loss_fct):
#     model.eval()
#     nlls = []
#     seqlen = args.seqlen
...
@torch.no_grad()
def evaluate(model, val_loader, loss_fct):
    model.eval()
    nlls = []
    total_samples = 0
...
if __name__ == "__main__":
    ...
    if args.model.startswith('facebook/opt'):
        ...
        model = prune_model(model, func=args.prune_level)
    ...
    if not args.full_tune:
        ...
        peft_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM, inference_mode=False, r=args.lora_dim, lora_alpha=32, lora_dropout=0.1
        )
    ...
    if args.dataset == "wikitext2":
        ...
        valenc = tokenizer("\n\n".join(raw_val_data['text']), return_tensors='pt')
        tot_num_val_seq = valenc['input_ids'].size(1) // args.seqlen
        valenc['input_ids'] = valenc['input_ids'][..., :tot_num_val_seq*args.seqlen]

        # def preprocess_function(examples):
        # # Tokenize the texts
        #     # TODO: should we truncate the text?
        #     result = tokenizer(examples["text"], padding=True, max_length=args.max_seq_length, 
        #                        truncation=True, return_tensors='pt')
        #     return result

        # tra_data = raw_tra_data.map(preprocess_function, batched=True, desc="Running tokenizer on dataset")
        # val_data = raw_val_data.map(preprocess_function, batched=True, desc="Running tokenizer on dataset")
        # tst_data = raw_tst_data.map(preprocess_function, batched=True, desc="Running tokenizer on dataset")

        # tra_data = tra_data.remove_columns("text")
        # val_data = val_data.remove_columns("text")
        # tst_data = tst_data.remove_columns("text")

        # train_dataloader = DataLoader(tra_data,
        ...
        pass
    elif args.dataset == 'ptb': # smallest
    ...
    optimizer_grouped_parameters = [
        {
            "params": model.parameters(), # [p for n, p in model.named_parameters() if n == "soft_prompt.weight"],
    ...
    #         if glb_step >0 and glb_step % args.eval_every_steps == 0:
    #             # TODO eval 
    #             val_ppl = evaluate(model, valenc, loss_fct)
    #             print(f'{val_ppl}: val_ppl')
    #             if val_ppl <= best_val_ppl:
    #                 torch.save(model.state_dict(),f"{ROOT}/{args.output_dir}/best.ckpt")
    #                 best_val_ppl = val_ppl
    #                 print(f"best val acc: {best_val_ppl}")
