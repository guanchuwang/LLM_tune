...
from approxlinear.fdmp import FDMP
...
class RMSNorm_func(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float16)
    def forward(ctx, input, weight, variance_epsilon, scheme):

        feature_pack = FDMP.fdmp1d(input, scheme)
        ctx.saved = feature_pack, input.shape, weight, variance_epsilon

        ctx.scheme = scheme
        res = RMSNorm_forward(weight, variance_epsilon, input)
        return res

    @staticmethod
    @custom_bwd
    def backward(ctx, grad_output):
        feature_pack, input_shape, weight, variance_epsilon = ctx.saved
        scheme = ctx.scheme

        input_hat = FDMP.de_fdmp1d(feature_pack, input_shape, scheme)
        grad_weight = grad_output * input_hat
        grad_input = grad_output * weight

        del feature_pack, input_shape, weight, variance_epsilon, input_hat, scheme
...
@torch.no_grad()
def RMSNorm_forward(weight, variance_epsilon, hidden_states):
